...
class Renderer(object):
    ...
    def glViewMatrix(self, translate = V3(0,0,0), rotate = V3(0,0,0)):
        ...
        self.viewMatrix = ml.MatInv(self.camMatrix)

    def glLookAt(self, eye, camPosition):
        forward = ml.subtractArrays([camPosition.x, camPosition.y, camPosition.z], [eye.x, eye.y, eye.z])
        forward[:] = [x / ml.norm(forward) for x in forward]

        
        
        right = ml.crossProduct(V3(0,1,0), forward)
        # right = right / ml.norm(right)
        right[:] = [x / ml.norm(right) for x in right]

        up = ml.crossProduct(forward, right)
        # up = up / ml.norm(up)
        up[:] = [x / ml.norm(up) for x in up]
    ...
    def glCamTransform(self, vertex):
        v = V4(vertex[0], vertex[1], vertex[2], 1)
        vt = ml.matrixMult(self.viewportMatrix, self.projectionMatrix)
        vt = ml.matrixMult(vt, self.viewMatrix)
        vt = ml.matMultVect(vt, v)
        vf = V3(vt[0] / vt[3],
                vt[1] / vt[3],
                vt[2] / vt[3])
    ...
    def glTriangle_bc(self, A, B, C, verts = (), texCoords = (), normals = (), clr = None):
        ...
        triangleNormal = ml.crossProduct( ml.subtractArrays([verts[1].x, verts[1].y, verts[1].z], [verts[0].x, verts[0].y, verts[0].z]), ml.subtractArrays([verts[2].x, verts[2].y, verts[2].z], [verts[0].x, verts[0].y, verts[0].z]))
        # normalizar
